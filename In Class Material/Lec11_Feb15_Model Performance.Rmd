---
title: "Feb 15, Assessing Model Performance"
...
*  Calculate the log pointwise divergence (lppd)   
...
genotype_freqs <- tibble(p=seq(from = 0.01, to = 0.99, by =0.01 ))%>%
  mutate(q = 1-p , 
         H = (-1)*(p^2 * log(p^2) + 2*p*q * log(2* p* q ) + q^2 * log(q^2)))
```
...
While lppd is the most accurate measure of out-of-sample entropy, other approximations are almost as good and *much* easier to calcuate. 
...
```{r}
compare(   m6.7 , m6.8 ,m6.7.2, func=WAIC )
```

```{r}
compare(   m6.7 , m6.8 ,m6.7.2, func=LOO )
```
IMPORTANT NOTE: This does not mean model 6.7.2 is the true model, it means that it predicts well without overfitting. 

Why does model 6.7 overfit? Because any in-sample association between treatment and outcome that is independent of fungus is spurious (i.e. it is random, not causal), and won't be shared with aout of sample data.
...
We make a random training dataset of $2^6=64$ fish parents where the number of eggs laid is random (Poisson) and the number that successfuly hatch is binomial with a hatching probability of 0.65. 
...
But we can also look at how lppd improves as we add parameters, i.e. as we split the data into more groups. 
```{r}
ggplot(out.long.effect, aes(x=par.indel,y=indel, group=par.indel ))+
  geom_violin()+
  labs(x="group being compared",
       y="Decrease in deviance")
```
Zooming in, we can see that the lppd is getting better for the models with more parameters. It basically has to get better, but now it is not getting better by more than the number of parameters times 2. 
```{r}
ggplot(out.long.effect, aes(x=par.indel,y=indel, group=par.indel ))+
  geom_violin()+
  ylim(c(0,400))+
