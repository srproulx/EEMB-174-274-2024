...
Specific funcitons must be used for each type of model. Here we want `stan_glmer`. 
```{r}
m1.6.stan_glmer <- stan_glmer( cbind(surv,density) ~ dtreat*pred + (1 | tank) ,
                            family = binomial,
                            prior = normal( 0, 1),
...
```{r}
summary(m1.6.stan_glmer, digits = 2)
...
m1.6.stan_glm <- stan_glm( cbind(surv,density) ~ dtreat*pred +   tank_f ,
                            family = binomial,
                            prior = normal( 0, 1),
                            prior_intercept = normal( 0, 1.5),
                            prior_aux = exponential(1),
