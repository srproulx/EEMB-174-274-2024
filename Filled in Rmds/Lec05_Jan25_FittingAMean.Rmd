...
posterior_table <- d_grid %>%
  mutate(log_likelihood=height_lik_f_vec(mu,sigma),
         log_prior_mu = dnorm(mu,mean=178,sd=20,log=TRUE), # Our prior is a formula based on a normal distribution with mean = 178 and sd = 20. You can use the dnorm function with log=TRUE to produce log probabilities.
...
```{r}
samples_height_model <- sample_n(posterior_table,size=1e4,weight=posterior,replace = TRUE) %>% # sample from your grid approximation posterior
  select(mu,sigma)
```
...
ggplot(data=samples_height_model, aes(x=mu,y=sigma)) + 
  geom_jitter(width=0.1,height=0.03,alpha=0.01)+
